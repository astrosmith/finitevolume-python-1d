...
def main():
	...
	while t < tEnd:
		...
		dt = courant_fac * np.min( dx / (np.sqrt( gamma*P/rho ) + np.sqrt(vx**2)) )
		plotThisTurn = False
		...
		rho_prime = rho - 0.5*dt * ( vx * rho_dx + rho * vx_dx * rho_dy + rho)
		vx_prime  = vx  - 0.5*dt * ( vx * vx_dx * vx_dy + (1/rho) * P_dx )
		P_prime   = P   - 0.5*dt * ( gamma*P * (vx_dx)  + vx * P_dx * P_dy )
		...
		# compute fluxes (local Lax-Friedrichs/Rusanov)
		flux_Mass_X, flux_Momx_X, flux_Momy_X, flux_Energy_X = getFlux(rho_XL, rho_XR, vx_XL, vx_XR, P_XL, P_XR, gamma)
		flux_Mass_Y, flux_Momy_Y, flux_Momx_Y, flux_Energy_Y = getFlux(rho_YL, rho_YR, vx_YL, vx_YR, P_YL, P_YR, gamma)
