...
def plot_snap(base='sod', num=0):
    num_str = '{0:04d}'.format(num)
    filename = '../../data/'+base+'/'+base+'_'+num_str+'.hdf5'
    plotname = 'fig_'+base+'_'+num_str+'.pdf'
    print '\n----------------------------------------' +\
          '\n filename: ' + filename                   +\
          '\n----------------------------------------\n'
    ...
    hydro = f['hydro']
    mesh  = f['mesh']
    print 'hydro.attrs.items() =', hydro.attrs.items()
    print 'mesh.attrs.items()  =', mesh.attrs.items()
    t = hydro.attrs['t']
    gamma = hydro.attrs['gamma']
    xi = mesh['r'][:]
    if mesh.attrs['coord_sys'] == 'spherical':
        xi -= xi[0]  # Shift back to the origin
    x  = 0.5 * (xi[:-1] + xi[1:])
    ui = mesh['u'][:]
    u  = 0.5 * (ui[:-1] + ui[1:])
    e  = mesh['e'][:]
    ...
    ## Plot data ##
    for ax,y,ya in zip(axs,ys,yas):
        ax.plot(d['x'], ya, lw=0.5)
        ax.scatter(x, y, marker='.', s=10, edgecolor='none', facecolor=[0.5, 0.5, 0.5])
